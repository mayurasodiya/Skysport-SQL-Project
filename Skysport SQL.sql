...
rename table sql_skysports_project16875987300 to Skysport1;

rename table sql_skysports_project16875987301 to Skysport2;
...
FROM Skysport1
WHERE goals_scored = (SELECT MAX(goals_scored) FROM Skysport1);
...
SELECT team, goals_scored 
FROM Skysport1    
WHERE goals_scored = (SELECT min(goals_scored) FROM Skysport1);
...
from Skysport2 
where players_used = (SELECT max(players_used) FROM Skysport2);
...
select team, fouls 
from Skysport2
where fouls=(select max(fouls) from Skysport2);
...
from Skysport2 td inner join Skysport1 gs 
on td.team=gs.team
group by gs.`group` 
